...
```shell
  docker buildx build -t aerabi/rekor:latest . --load
```

To install the extension, use `make install-extension` **or**:

```shell
  docker extension install aerabi/rekor:latest
```
...
```shell
  docker extension dev ui-source aerabi/rekor:latest http://localhost:3000
```

In order to open the Chrome Dev Tools for your extension when you click on the extension tab, run:

```shell
  docker extension dev debug aerabi/rekor:latest
```

Each subsequent click on the extension tab will also open Chrome Dev Tools. To stop this behaviour, run:

```shell
  docker extension dev reset aerabi/rekor:latest
```
...
```shell
docker extension update aerabi/rekor:latest
```
...
```shell
docker extension rm aerabi/rekor:latest
```
